...
for(int i=0;i<size;i++){
    ...
    for(int j=i;j<size;j++){
        s+=arr[j];
        if(s==sum){
            System.out.println("The sum of Elemens form index "+(+1)+" position to "+(j+1)+" position is  "+sum);
            while(i<=j){
...
if(subArraySum>sum){System.out.println("sum of sub array "+subArraySum);return;}
    }
}
...
public static void main(String[] args) {
   ...
   for(int i:subArraySum)System.out.print(i+" ");
